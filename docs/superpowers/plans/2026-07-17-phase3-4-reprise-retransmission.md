...
- **Retenu — défense en profondeur à 3 couches, calquée sur celle de l'IN (`ereporting-sweep.service.ts:56-78`)** :
  ...
  - **Couche 2 (`jobId` déterministe)** : `${declarantId}:${fluxKind}:${periodStart}:RE:${reSeq}` — BullMQ déduplique les ré-enfilements tant que le job existe (anti-double-clic dans la fenêtre de rétention).
