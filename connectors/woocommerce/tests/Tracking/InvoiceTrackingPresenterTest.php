...
namespace FactelecWoo\Tests\Tracking;

use FactelecWoo\Tracking\InvoiceTrackingPresenter;
use PHPUnit\Framework\TestCase;

/**
 * InvoiceTrackingPresenter — logique pure (aucune classe WP/WC référencée),
 * construit la vue affichée dans la metabox admin commande à partir de la
 * liaison locale (`factelec_invoice_link`) et, si demandé, du statut
 * distant (`FactelecClient::getInvoiceStatus()`, déjà testé tâche 1). Mêmes
 * scénarios que connectors/prestashop/tests/Tracking/
 * InvoiceTrackingPresenterTest.php.
 */
final class InvoiceTrackingPresenterTest extends TestCase
{
    ...
    /**
     * @return array{id: int, order_id: int, invoice_id: ?string, status: string, last_error: ?string}
     */
